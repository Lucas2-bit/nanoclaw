      ...
      if (result.result) {
        ...
        if (text) {
          try {
            ...
            recordSendFailure(chatJid, text, group.folder, err);
...
async function main(): Promise<void> {
  ...
  // Start session file size monitor (Fix 1 — OOM prevention)
  // On critical threshold, send a WhatsApp alert to the main group
  startSessionMonitor(
    () => registeredGroups,
    (groupFolder, sizeKB) => {
      ...
      channel
        .sendMessage(
          mainJid,
          `⚠️ Session for *${groupFolder}* is ${sizeKB} KB — approaching OOM limit. Send /compact to that group now.`,
        )
        .catch((err) =>
          logger.warn(
            { err },
            'session-monitor: failed to send critical alert',
          ),
        );
    },
  );
