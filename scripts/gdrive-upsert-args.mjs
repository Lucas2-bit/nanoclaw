...
// Usage: node gdrive-upsert-args.mjs <folderId> <file1> [file2 ...]
const [folder, ...files] = process.argv.slice(2);
if (!folder || files.length === 0) {
  console.error('Usage: node gdrive-upsert-args.mjs <folderId> <file...>');
  process.exit(1);
}
...
for (const filePath of files) {
  ...
  try {
    ...
    results.push({ name: res.data.name, id: res.data.id, link: res.data.webViewLink, action, folder });
